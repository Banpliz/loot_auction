...
//
// Rewritten 2026-09-01 (round 3) after two live tests: round 1's shape-based
// description still let rank badges and background chat text through; round 2 (spatial
// anchoring — inside the panel, right of the boss name) fixed that, but the model then
// over-segmented each real icon into 2-3 separate "items" — boxing the level pill
// ("40 ур."), the "Уник." tag, and boss-name text fragments as if each were its own
// icon, alongside the real ones. Round 3 adds an explicit "one icon = one box, nothing
// below or around it gets its own box" rule and names the level pill / tag specifically,
// since generic "ignore text" wasn't specific enough to stop them being boxed too.
...
3. Правее, в конце строки — сама группа иконок наград: несколько небольших иконок подряд, у каждой контрастная цветная РАМКА-ромб/шестиугольник (синяя, фиолетовая или красная), внутри картинка предмета или сундука. Именно ЭТИ иконки (пункт 3) и нужно вернуть — по одной рамке на каждую, и БОЛЬШЕ НИЧЕГО.

ОЧЕНЬ ВАЖНО про то, что считается ЧАСТЬЮ иконки, а не отдельным элементом: под иконкой часто есть подпись уровня («40 ур.», цветная плашка) и/или зелёный тег «Уник.» — это НЕ отдельные иконки и не награды сами по себе, у них никогда не должно быть своей отдельной рамки. Если они есть — они либо входят внутрь рамки той иконки, к которой относятся, либо вообще не включаются в ответ. Никогда не возвращай рамку, которая содержит ТОЛЬКО текст/плашку уровня, ТОЛЬКО тег «Уник.», ИЛИ только буквы/часть слова без картинки предмета внутри — если в рамке нет самой картинки предмета/сундука, это не иконка награды, не включай её.

Одна иконка = одна рамка, тесно вокруг картинки предмета с её цветной рамкой редкости и, если виден, числом-бейджиком «×N» в углу. Не создавай отдельные рамки для соседних иконок, подписей уровня, тегов или кусков текста рядом.
...
Если по какой-то строке ты не уверен, что там реальная иконка награды (а не значок места, не текст, не подпись уровня/тег, не фон) — просто пропусти эту строку/элемент. Лучше вернуть меньше элементов, чем ошибочно включить значок места, подпись или кусок постороннего текста/чата.
